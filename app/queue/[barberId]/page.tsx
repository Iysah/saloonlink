...
const QueuePage = () => {
  ...
  useEffect(() => {
    if (barberId) {
      ...
      const queueSubscription = supabase
        ...
        .on('postgres_changes', 
          { event: '*', schema: 'public', table: 'queue', filter: `barber_id=eq.${barberId}` },
          () => fetchQueue()
        )
        .subscribe();
  ...
  const fetchQueue = async () => {
    ...
    if (data) {
      ...
      // Check if user is already in queue (by phone number)
  ...
  const joinQueue = async (e: React.FormEvent) => {
    ...
    try {
      // Check if phone is already in queue
      ...
      const estimatedWait = nextPosition * 30; // 30 minutes per person estimate
