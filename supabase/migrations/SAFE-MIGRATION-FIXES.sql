-- SAFE MIGRATION: Fix all RLS policy issues (handles existing policies)
...
-- ========================================
-- 4. CLEAN UP DUPLICATE PROFILES (SAFE)
-- ========================================
-- Only delete if there are actually duplicates
...
-- ========================================
-- 5. CLEAN UP ORPHANED RECORDS (SAFE)
-- ========================================
-- Only delete if there are actually orphaned records
...
-- ========================================
-- 6. ADD CONSTRAINTS TO PREVENT FUTURE DUPLICATES (SAFE)
-- ========================================
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'barber_profiles_user_id_unique' 
        AND table_name = 'barber_profiles'
    ) THEN
        ALTER TABLE barber_profiles ADD CONSTRAINT barber_profiles_user_id_unique UNIQUE (user_id);
    END IF;
END $$;
...
-- ========================================
-- 8. ADD INDEXES FOR BETTER PERFORMANCE (SAFE)
-- ========================================
...
SELECT 
  ...
  with_check
FROM pg_policies 
WHERE tablename IN ('profiles', 'services', 'queue')
...
-- ✅ Cleaned up duplicate profiles (if any existed)
-- ✅ Cleaned up orphaned records (if any existed)
-- ✅ Added constraints to prevent future duplicates (if they didn't exist)
-- ✅ Created database trigger for automatic profile creation
-- ✅ Added performance indexes (if they didn't exist)
-- ✅ Added documentation comments
-- ✅ Added verification queries to confirm success 